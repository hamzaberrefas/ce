											...
											<tr className="product-list" key={Math.random()}>
												...
												<td>
													<div className="flex items-center">
														<div className="me-2">
															<span className="avatar avatar-md !rounded-none">
																<img src={idx.src} alt="" className="!rounded-none" />
															</span>
														</div>
												...
												<td>{idx.price}</td>
												<td className={Number(idx.stock) < 10 ? 'text-rose-600' : ''}>{idx.stock}</td>
												<td>{idx.gender}</td>
												<td>{idx.seller}</td>
												<td>{idx.date}</td>
												<td>
													<div className="flex flex-row items-center !gap-2 text-[0.9375rem]">
														<Link aria-label="anchor" to={`${import.meta.env.BASE_URL}dashboard/products/edit`}
															className="ti-btn ti-btn-wave  !gap-0 !m-0 !h-[1.75rem] !w-[1.75rem] text-[0.8rem] text-info hover:bg-info hover:text-white hover:border-info !border !border-info !rounded-full"><i
																className="ri-pencil-line"></i></Link>
														<Link aria-label="anchor" to="#" onClick={() => handleDelete(idx.id)}
															className="ti-btn ti-btn-wave product-btn !gap-0 !m-0 !h-[1.75rem] !w-[1.75rem] text-[0.8rem] text-danger hover:bg-danger hover:text-white hover:border-danger !border !border-danger !rounded-full"><i
																className="ri-delete-bin-line"></i></Link>
														<Link aria-label="anchor" to="#" onClick={() => { }}
															className="ti-btn ti-btn-wave product-btn !gap-0 !m-0 !h-[1.75rem] text-[0.8rem] text-primary hover:bg-primary hover:text-white !border !border-primary !rounded-full">
