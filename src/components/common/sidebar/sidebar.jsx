      ...
      if (!event?.ctrlKey) {
        for (const item of MENUITEMS) {
          if (item === targetObject) {
            ...
            item.active = true; // Set active to false for items not matching the target
    ...
    for (const mutation of mutationsList) {
      if (mutation.type === 'attributes' && (mutation.attributeName === 'data-nav-layout' || mutation.attributeName === 'data-vertical-style')) {
        ...
        if (newValue == 'vertical') {
          let currentPath = location.pathname.endsWith('/') ? location.pathname.slice(0, -1) : location.pathname;
          currentPath = !currentPath ? '/dashboard/analytics' : currentPath;
          setMenuUsingUrl(currentPath);
        } else {
          closeMenuFn();
        }
      ...
      <aside className="app-sidebar" id="sidebar" onMouseEnter={() => Onhover()}
        ...
        <div className="main-sidebar-header">
          <a href={`${import.meta.env.BASE_URL}`} className="header-logo">
            <img src={logo1} alt="logo" className="desktop-logo" style={{ width: '115px', height: 'auto' }} />
            <img src={logo2} alt="logo" className="toggle-logo" />
            <img src={logo3} alt="logo" className="desktop-dark" style={{ width: '115px', height: 'auto' }} />
            <img src={logo4} alt="logo" className="toggle-dark" />
            <img src={logo5} alt="logo" className="desktop-white" style={{ width: '115px', height: 'auto' }} />
